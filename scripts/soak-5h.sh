#!/usr/bin/env bash
# Soak test 5h — monitora, reconecta Wi-Fi, reinicia companion se morrer.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
LOG="${ROOT}/cozmo-soak-5h.log"
DUR_S="${1:-18000}"
INTERVAL="${COZMO_SOAK_INTERVAL_S:-60}"
END=$(( $(date +%s) + DUR_S ))
LOCK="/tmp/cozmo-companion.lock"
FLOCK="/tmp/cozmo-soak-5h.lock"
ALERTS=0
CRITICOS=0

exec 9>"$FLOCK"
if ! flock -n 9; then
  echo "[$(date -Iseconds)] soak já em execução — abortando duplicata" | tee -a "$LOG"
  exit 0
fi
...
log_metrics() {
  local tail_n="${1:-400}"
  local gov cozmo01 stall drx dtx rd fase
  gov="$(tail -n "$tail_n" "$ROOT/cozmo-companheiro.log" 2>/dev/null | grep 'Governador fase=' | tail -1 | tr -d '\n' || true)"
  cozmo01="$(tail -n "$tail_n" "$ROOT/cozmo-companheiro.log" 2>/dev/null | grep -c 'COZMO 01' 2>/dev/null || echo 0)"
  stall="$(tail -n "$tail_n" "$ROOT/cozmo-companheiro.log" 2>/dev/null | grep -c 'rx=STALL' 2>/dev/null || echo 0)"
  cozmo01="${cozmo01//[$'\n\r ']/}"
  stall="${stall//[$'\n\r ']/}"
  ...
  echo "$cozmo01|$stall|$drx|$dtx|$rd|$fase|$gov"
}

INICIO=$(date +%s)
log "=== SOAK inicio dur=${DUR_S}s interval=${INTERVAL}s pid=$$ ==="
...
while [[ $(date +%s) -lt $END ]]; do
  ...
  IFS='|' read -r cozmo01 stall drx dtx rd fase gov <<<"$(log_metrics 400)"
  log "t=${elapsed}s pid=${pid:-?} ping=${ping_s} drx=${drx:-?} dtx=${dtx:-?} rd=${rd:-?} fase=${fase:-?} cozmo01=${cozmo01} stall=${stall} alerts=${ALERTS} criticos=${CRITICOS}"

  if [[ "${stall:-0}" -gt 5 ]] || [[ "${cozmo01:-0}" -gt 0 ]]; then
    CRITICOS=$((CRITICOS + 1))
...
log "=== SOAK fim elapsed=$(( $(date +%s) - INICIO ))s alerts=${ALERTS} criticos=${CRITICOS} ==="
